...
if (NOT TARGET counter_test)
    add_library (counter_test INTERFACE)
    target_link_libraries (counter_test INTERFACE GTest::gtest_main counter_lib)
    add_library (counter::test ALIAS counter_test)
endif ()

# Add the check target that builds and runs tests.
add_custom_target (check COMMAND ${CMAKE_CTEST_COMMAND} ${CMAKE_CTEST_ARGUMENTS})

get_directory_property (counter_targets DIRECTORY "${counter_SOURCE_DIR}/src" BUILDSYSTEM_TARGETS)
foreach (target IN LISTS counter_targets)
    get_target_property (type ${target} TYPE)
    if (type STREQUAL "EXECUTABLE")
        list (APPEND counter_EXECUTABLE_LIST ${target})
    endif ()
endforeach ()
unset (counter_targets)

macro (add_app_test test_filename)
    file (RELATIVE_PATH source_file "${counter_SOURCE_DIR}" "${CMAKE_CURRENT_LIST_DIR}/${test_filename}")
    get_filename_component (target "${source_file}" NAME_WE)

    add_executable (${target} ${test_filename})
    target_link_libraries (${target} counter::test)

    add_dependencies (${target} ${counter_EXECUTABLE_LIST})
